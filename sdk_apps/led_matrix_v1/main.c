 ...
 * matrix (device name "LEDMATRIX0") with two modes:
 ...
 *   y=160..259  Mode selector
 *   y=280..379  Text input box
 *   y=400..     Key hints
 */
...
/* -------------------------------------------------------------------------
 * Simple on-screen bitmap font for UI text (3x5, packed as column bytes).
 * We use SDL_RenderDebugText which is available in SDL3 for basic ASCII.
 * ------------------------------------------------------------------------- */
...
#define LED_SIZE        40   /* pixel size of each LED square */
#define LED_GAP         8    /* gap between LEDs */
#define LED_CELL        (LED_SIZE + LED_GAP)  /* total cell size */

/* Total matrix pixel dimensions */
#define MATRIX_PX_W     (MATRIX_COLS * LED_CELL - LED_GAP)
#define MATRIX_PX_H     (MATRIX_ROWS * LED_CELL - LED_GAP)

/* Where to draw the matrix preview */
...
/* -------------------------------------------------------------------------
 * Build a 5-column pixel buffer for the current view.
 * columns[c] is a uint8_t bitmask: bit r = row r (bit 0 = top row).
 * The font uses rows 0-6 (7 rows); row 7 is always 0.
 * ------------------------------------------------------------------------- */
static void get_columns_static(AppState *s, uint8_t cols[5]) {
    ...
    for (int c = 0; c < 5; c++) {
        /* font bits 0-6 = rows 0-6; row 7 = 0 */
        cols[c] = glyph[c] & 0x7F;
    }
}

/*
 * Build a virtual pixel column from the scroll buffer.
 * Each character is 5 columns wide + 1 blank column gap.
 * total_width = text_len * 6.
 * col_pixel is an absolute pixel column index (0..total_width-1).
 */
static uint8_t scroll_col_at(AppState *s, int col_pixel) {
    ...
    if (col_in_char == 5) return 0; /* gap column */
...
static void get_columns_scroll(AppState *s, uint8_t cols[5]) {
    for (int c = 0; c < 5; c++) {
        cols[c] = scroll_col_at(s, s->scroll_x + c);
    }
}
    ...
    for (int col = 0; col < MATRIX_COLS; col++) {
        for (int row = 0; row < MATRIX_ROWS; row++) {
            ...
            if (on) {
                ...
                SDL_SetRenderDrawColor(r, 0x1A, 0x1A, 0x1A, SDL_ALPHA_OPAQUE);
            }
...
/* Draw a filled rectangle border */
...
/* Render debug text centered at (cx, y) */
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    ...
    *appstate = s;
    ...
    /* Try to get the LED matrix device */
    s->led_matrix = device_get("LEDMATRIX0");
    if (!s->led_matrix) {
        SDL_Log("LEDMATRIX0 not found — running in preview-only mode");
    }
        ...
        case SDL_EVENT_KEY_DOWN: {
            ...
            switch (sc) {
                ...
                case SDL_SCANCODE_RIGHT:
                    if (s->mode == 0 && s->text_len > 0) {
                        s->static_char_idx++;
                        if (s->static_char_idx >= s->text_len)
                            s->static_char_idx = 0;
                        update_matrix(s);
                    }
                    break;
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    /* ---- Text input area (y=280..379) ---- */
    {
        ...
        if (s->text_len > 0) {
            ...
            /* Clamp display to fit box: show last N chars */
            int max_chars = 70;
            const char *display = s->text;
            if (s->text_len > max_chars) {
                display = s->text + (s->text_len - max_chars);
            }
            ...
            SDL_SetRenderDrawColor(r, 0x44, 0x44, 0x44, SDL_ALPHA_OPAQUE);
            SDL_RenderDebugText(r, 70.0f, 312.0f, "(empty)");
            if (s->editing) {
                SDL_RenderDebugText(r, 70.0f, 312.0f, "_");
            }
        }
    ...
    /* ---- Static mode: char index indicator ---- */
    ...
    /* ---- Key hints (y=400..) ---- */
    {
        ...
        if (s->mode == 0) {
            render_text_centered(r, 360.0f, 410.0f,
                "LEFT/RIGHT navigate chars   T edit text   M toggle mode   ESC quit");
        } else {
            render_text_centered(r, 360.0f, 410.0f,
                "T edit text   M toggle mode   ESC quit");
        }
    }
